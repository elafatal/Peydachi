...
async def review_deleted_pic(deleted_pic_id: ID_BODY, db: DB_DEPENDENCY):
    return await deleted_pictures_function.review_deleted_pic(deleted_pic_id=deleted_pic_id, db=db)
...
async def delete_all_reviewed_deleted_pics(db: DB_DEPENDENCY):
    return await deleted_pictures_function.delete_all_reviewed_deleted_pics(db=db)
